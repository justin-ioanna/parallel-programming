...
package object scalashop extends BoxBlurKernelInterface {
  ...
  def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA = {
    ...
    val pixels = for {
      i <- clamp(x - radius, 0, src.width - 1) to clamp(x + radius, x, src.width - 1)
      j <- clamp(y - radius, 0, src.height - 1) to clamp(y + radius, y, src.height - 1)
    } yield src(i, j)

    val (r, g, b, a) = pixels.foldLeft((0, 0, 0, 0)) { (acc, pixel) =>
